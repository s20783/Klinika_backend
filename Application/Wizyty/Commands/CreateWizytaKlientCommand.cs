...
{
    public class CreateWizytaKlientCommand : IRequest<int>
    {
    ...
    public class UmowWizyteKlientCommandHandler : IRequestHandler<CreateWizytaKlientCommand, int>
    {
        private readonly IKlinikaContext _context;
        private readonly IHash _hash;
        private readonly IWizyta _wizyta;
        private readonly IEmailSender _sender;
        public UmowWizyteKlientCommandHandler(IKlinikaContext klinikaContext, IHash hash, IWizyta wizyta, IEmailSender emailSender)
        {
        ...
        public async Task<int> Handle(CreateWizytaKlientCommand req, CancellationToken cancellationToken)
        {
            int id1 = _hash.Decode(req.ID_klient);
            int id_harmonogram = _hash.Decode(req.ID_Harmonogram);

            if (!_wizyta.IsWizytaAbleToCreate(_context.Wizyta.Where(x => x.IdOsoba == id1).ToList()))
            {
            ...
            using (var transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                try
                {
                    ...
                    await _sender.SendUmowWizytaEmail(to, harmonogram.DataRozpoczecia, weterynarz);
