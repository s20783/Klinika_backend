...
{
    public class WizytaAdminQuery : IRequest<List<GetWizytaListResponse>>
    {
        public string ID_klient { get; set; }
    }

    public class WizytaAdminQueryHandler : IRequestHandler<WizytaAdminQuery, List<GetWizytaListResponse>>
    {
        private readonly IKlinikaContext _context;
        private readonly IHash _hash;
        public WizytaAdminQueryHandler(IKlinikaContext klinikaContext, IHash hash)
        {
            _context = klinikaContext;
            _hash = hash;
        }

        public async Task<List<GetWizytaListResponse>> Handle(WizytaAdminQuery req, CancellationToken cancellationToken)
        {
            ...
            return (from x in _context.Wizyta
                    ...
                    select new GetWizytaListResponse()
                    {
                        ...
                        Data = g.Key.WeterynarzIdOsoba != 0 ? _context.Harmonograms.Where(x => x.IdWizyta.Equals(g.Key.IdWizyta)).OrderBy(x => x.DataRozpoczecia).Select(x => x.DataRozpoczecia).First() : null
                    })
                    .AsParallel()
